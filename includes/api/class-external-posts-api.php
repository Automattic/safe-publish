...
class External_Posts_API {
	...
	private function prepare_post_for_listing( array $post, string $post_type = 'posts' ): array|false {
		...
		$prepared_post = array(
			...
			'title'          => isset( $post['title']['rendered'] ) ? html_entity_decode( sanitize_text_field( wp_strip_all_tags( $post['title']['rendered'] ) ), ENT_QUOTES | ENT_HTML5, 'UTF-8' ) : __( 'No Title', 'safe-publish' ),
