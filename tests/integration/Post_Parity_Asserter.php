...
final class Post_Parity_Asserter {
	...
	private const PLUGIN_ADDED_META = array(
		...
		'_thumbnail_id'                   => 'dest featured-attachment ID (resolution-checked)',
	);
	...
	private const ATTACHMENT_DIVERGENCE_REGISTRY = array(
		'ID'                => 'namespace difference (source vs dest WordPress)',
		'post_date'         => 'set to dest upload time, source media date not propagated',
		'post_date_gmt'     => 'set to dest upload time, source media date not propagated',
		'post_modified'     => 'set to dest upload time, source modified time not propagated',
		'post_modified_gmt' => 'set to dest upload time, source modified time not propagated',
		'guid'              => 'regenerated by dest WordPress upload path',
		'post_author'       => 'inherits dest current user, source media author not propagated',
		'post_status'       => "dest sets 'inherit'; source attachment status not propagated",
		'post_parent'       => 'inline/featured imports leave attachment unattached (parent=0)',
	);
	...
	private const ATTACHMENT_DEFERRED_REGISTRY = array(
		...
		'post_name'    => 'deferred: dest derives slug from filename; source slug not propagated',
	);
	...
	private const PLUGIN_ADDED_FEATURED_ATTACHMENT_META = array(
		...
		Options::META_MEDIA_TYPE        => "value 'featured_image' marks the attachment as featured-sourced",
	);
	...
	private const ATTACHMENT_META_DIVERGENCE = array(
		'_wp_attached_file'       => 'relative path under dest uploads dir; source path not propagated',
		'_wp_attachment_metadata' => 'sub-size dimensions and paths regenerated by dest WordPress',
	);
