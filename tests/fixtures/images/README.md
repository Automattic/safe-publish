...
WordPress's `download_url()` creates empty temp files even when HTTP responses contain data. The `fix_empty_temp_files()` method detects empty files and copies fixture content to them before `media_handle_sideload()` validates the file.
