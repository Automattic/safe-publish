...
- **Posts** — local posts that resulted from successful imports. The tab is scoped to the connected source site, so changing the connection hides the previous site's rows without deleting them, and reconnecting brings them back. Each row offers Edit, Update (re-import from source), Diff (compare current local content with current source content), Delete, and Rollback. Update, Delete, and Rollback can also be applied to several selected rows at once; Diff is available per row only. A **Sync Status** column compares each row against the current source content and reports whether it is up to date, outdated, missing on the source, cannot be checked because the source request failed, or has an invalid timestamp.
