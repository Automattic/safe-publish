...
The Compare action on the Imports → Posts tab compares the current local post with the current source content, covering the title, content, excerpt, featured image, metadata, and taxonomy terms, including each term's parent and description. It can be viewed inline or side by side, and for block-editor content the diff is computed block by block so editors can see exactly which blocks were added, removed, or changed. Only the taxonomies the source sends are compared, since the import touches no others; a taxonomy this site registers and the source omits is not reported as a removal. Differences the import would not apply are noted under the comparison: a term difference names the term and the field it affects, and a taxonomy this site does not register names the taxonomy. The modal also offers an Update button that re-imports the post from the source.
