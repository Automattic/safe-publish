...
add_action( 'init', function (): void {
    ...
    foreach ( $keys as $key => $type ) {
        register_post_meta( '', $key, array(
            ...
            'single'        => true,
            'type'          => $type,
            // Gate to users who can already edit the post. Safe Publish reads
            // the source with context=edit as an editor-capable user, so this
            // is enough for the value to appear in the response.
...
Register the keys against every post type whose SEO settings should migrate. Passing `''` as the object subtype registers them globally, which is the simplest choice; pass a specific post type instead if you want to scope it.
