...
> **Developer note:** The **Not imported** label maps to the internal `available` value used in query parameters, AJAX payloads, and code. It covers posts with no active successful import — never imported, error-only, or every import rolled back — and imported posts whose destination post is missing or trashed. A trashed destination is distinguished by the Local Status column, which reads **Trashed**; importing such a row is normally refused. The column follows the import history row, while the refusal follows the source post ID in post metadata, so the two can disagree when more than one post claims a source post. Local State is derived from import history rather than stored in a dedicated database field. Retaining `available` preserves backward compatibility.
...
| Local Status | Destination `post_status`, **Trashed** when the destination post is in the trash, or a dash when there is no destination post. |
